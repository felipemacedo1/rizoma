...
jar="$project_dir/rizoma-cli/target/rizoma-cli-0.1.0-SNAPSHOT-all.jar"
...
start=$(date +%s)
/usr/bin/time -v -o "$metrics" java -Xms32m -Xmx256m -jar "$jar" analyze "$csv" --schema "$schema" --out "$report" --delimiter semicolon --header first
end=$(date +%s)
...
printf '%s\n' 'heap_limit=-Xmx256m; maximum resident set size is total process RSS, not heap usage'
