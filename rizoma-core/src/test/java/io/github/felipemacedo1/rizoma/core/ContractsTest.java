...
class ContractsTest {
    ...
    @Test void ergonomicSchemaBuildersKeepDefaultsAndProduceImmutableContracts() {
        var document = TargetField.builder("customer.document")
                .name("Documento")
                .type(PhysicalType.TEXT)
                .aliases("CPF", "Documento Cliente")
                .semanticType("br:cpf")
                .required()
                .exclusive(false)
                .build();
        var code = TargetField.builder("code").build();
        var schema = TargetSchema.builder("customer")
                .version("2")
                .context("onboarding")
                .locale("pt-BR")
                .field(document)
        ...
        assertEquals("customer.document", document.id());
        assertEquals(List.of("CPF", "Documento Cliente"), document.aliases());
        assertEquals(Set.of(new SemanticType("br:cpf")), document.semanticTypes());
        assertTrue(document.required());
        assertFalse(document.exclusive());
        assertEquals("code", code.displayName());
        assertEquals(PhysicalType.TEXT, code.physicalType());
        assertEquals("2", schema.version());
        assertEquals("onboarding", schema.context());
        assertEquals("pt-BR", schema.locale());
        assertThrows(UnsupportedOperationException.class, () -> schema.fields().add(code));
        assertThrows(IllegalArgumentException.class, () -> TargetField.builder(" "));
        assertThrows(IllegalArgumentException.class, () -> TargetField.builder("x").aliases(" "));
        assertThrows(IllegalArgumentException.class, () -> TargetSchema.builder("x").build());
    }
